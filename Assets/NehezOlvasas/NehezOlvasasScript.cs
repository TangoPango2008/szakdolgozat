...
public class NehezOlvasasScript : MonoBehaviour
{
    ...
    private const string BestTimesFile = "Assets/best_times_hard_read.txt";
    ...
    void Start()
    {
        ...
        TextAsset textAsset = Resources.Load<TextAsset>("HardObjects"); // Load without the .txt extension
        if (textAsset != null)
        {
        ...
        {
            Debug.LogError("Failed to load HardObjects.txt from Resources.");
        }

        LoadBestTimes();

        Sprite[] FindableObjects = Resources.LoadAll<Sprite>("OlvasasNehezTargyak");

        // Shuffle the array to randomize the objects
        System.Random random = new System.Random();
        FindableObjects = FindableObjects.OrderBy(x => random.Next()).ToArray();

        // Limit the number of objects spawned to 15
        int maxObjects = Mathf.Min(15, FindableObjects.Length);
    ...
    void LoadBestTimes()
    {
        string persistentFilePath = Path.Combine(Application.persistentDataPath, "best_times_hard_read.txt");
        ...
        else
        {
            ...
            TextAsset textAsset = Resources.Load<TextAsset>("best_times_hard_read");
            if (textAsset != null)
            {
