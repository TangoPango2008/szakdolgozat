...
public class KozepesOlvasasScript : MonoBehaviour
{
    ...
    private const string BestTimesFile = "Assets/best_times_medium_read.txt";
    ...
    void Start()
    {
        ...
        TextAsset textAsset = Resources.Load<TextAsset>("MediumObjects"); // Load without the .txt extension
        if (textAsset != null)
        {
        ...
        {
            Debug.LogError("Failed to load MediumObjects.txt from Resources.");
        }


        LoadBestTimes();

        Sprite[] FindableObjects = Resources.LoadAll<Sprite>("OlvasasKozepesTargyak");

        // Shuffle the array to randomize the objects
        System.Random random = new System.Random();
        FindableObjects = FindableObjects.OrderBy(x => random.Next()).ToArray();

        // Limit the number of objects spawned to 12
        int maxObjects = Mathf.Min(12, FindableObjects.Length);
    ...
    void LoadBestTimes()
    {
        string persistentFilePath = Path.Combine(Application.persistentDataPath, "best_times_medium_read.txt");
        ...
        else
        {
            ...
            TextAsset textAsset = Resources.Load<TextAsset>("best_times_medium_read");
            if (textAsset != null)
            {
